...
def test(step_idx, policy_net, config):
    ...
    for _ in range(num_test):
        ...
        if episode_scores:
            ...
            avg_episodes_scores.append(np.mean(episode_scores))
        else:
            final_episode_scores.append(0.0)
            avg_episodes_scores.append(0.0)

        test_scores.append(episode_score)

    avg_score = np.mean(test_scores)
    avg_final_episode_score = np.mean(final_episode_scores)
    avg_avg_episodes_score = np.mean(avg_episodes_scores)
    print(
        f"Step # :{step_idx}, avg test score : {avg_score}, avg avg episodes score : {avg_avg_episodes_score}, avg final episode score : {avg_final_episode_score}"
    )

    env.close()
    return avg_score, avg_final_episode_score, avg_avg_episodes_score
...
def main(config: TrainingConfig, writer: SummaryWriter, envs: ParallelEnv):
    ...
    for policy_update_idx in range(0, config.num_policy_updates):
        ...
        # Run evaluation
        avg_score, avg_final_episode_score, avg_avg_episode_return = test(
            policy_update_idx, policy_net, config
        )
        writer.add_scalar("charts/test_mean_reward", avg_score, policy_update_idx)
        writer.add_scalar(
            "charts/test_mean_final_episode_return",
            avg_final_episode_score,
            policy_update_idx,
        )
        writer.add_scalar(
            "charts/test_mean_avg_episode_return",
            avg_avg_episode_return,
            policy_update_idx,
        )
