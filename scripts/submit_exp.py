...
# fmt: off
parser = argparse.ArgumentParser(description='RL Experiment Submission')
# experiment generation
parser.add_argument('--exp-script', type=str, default="debug.sh",
    help='the file name of this experiment')
parser.add_argument('--command', type=str, default="poetry run python train.py",
...
if args.build:
    ...
    subprocess.run(
        f"docker build --platform {args.archs} -t {args.docker_tag} .",
        shell=True,
        check=True,
    )
...
if args.provider == "aws":
    ...
    for final_run_cmd in final_run_cmds:
        job_name = args.docker_tag.replace(":", "").replace("/", "_").replace(
            " ", ""
        ).replace("-", "_") + str(int(time.time()))
